...
<body>
<div class="container well login_block_inst" align="center">
	...
	<div class="row">
		<form ng-app="myapp" ng-controller="BrijController" name="myForm"  novalidate>
			...
			<td><input type="email" class="form-control" name="s_email" placeholder="Enter Email" ng-model="s_email" ng-style="emailStyle" ng-change="analyze5(s_email)" onKeyUp="check_exists('s_bemail',this.value)" onBlur="check_exists('s_bemail',this.value)" required></td>
			<td>
			...
			<div class="form-group">
			<tr>
			<td><label for="s_buser">Business Name:</label><br></td>
			<td><input type="text" class="form-control" name="s_buser" placeholder="Enter Business Nname" ng-model="s_buser"  ng-style="buserStyle" ng-change="analyze6(s_buser)" required  user-dir></td>
			<td>
			<span style="color:red" id="s_buser" ng-show="myForm.s_buser.$dirty && myForm.s_buser.$invalid">
			<span ng-show="myForm.s_buser.$error.required">Business name is required</span>
			<span ng-show="!myForm.s_buser.$error.required && myForm.s_buser.$error.uservalid">Enter more than 3 characters</span>
			</span>
			</td>
			</tr>
			</div>
			<div class="form-group">
			<tr>
			<td><label for="s_bemail">Business Email:</label><br></td>
			<td><input type="email" class="form-control" name="s_bemail" placeholder="Enter Business Email" ng-model="s_bemail"  ng-style="bemailStyle" onKeyUp="check_exists('s_bemail',this.value)"  onBlur="check_exists('s_bemail',this.value)" ng-change="analyze7(s_bemail)" required></td>
			<td>
			<span style="color:red;" id="s_bemail" ng-show="myForm.s_bemail.$dirty">
			</span>
			</td>
			</tr>
			</div>
			...
			<tr>
			<p>
			<td><input type="submit" onClick="check_details()" id="submit_btn" class="btn btn-primary" ng-disabled="myForm.s_user.$invalid ||  myForm.s_email.$invalid ||  myForm.s_password.$invalid ||  myForm.s_cpassword.$invalid ||   myForm.s_mobile.$invalid || myForm.s_buser.$invalid ||  myForm.s_bemail.$invalid"></td>
			...
			<tr>
				<td>
					<a class="alert-link" href="institute.php" >have a account?</a>
				</td>
				<td></td>
				<td></td>
			</tr>
...
<script>
	...
	myApp.controller("BrijController", function($scope,$http) {
                ...
                $scope.analyze6 = function(value) {
                    if(value.length>3) {
                        $scope.buserStyle["border-color"] = "green";
						
                    } 
					else {
                        $scope.buserStyle["border-color"] = "red";
                    }
                };
                ...
                $scope.analyze7 = function(value) {
					var patt3=new RegExp("^[a-z0-9._]+@[a-z]+\.[a-z.]{2,5}$");
                    if(patt3.test(value)) {
                        $scope.bemailStyle["border-color"] = "green";
                    } 
					else {
                        $scope.bemailStyle["border-color"] = "red";
                    }
                };
...
function check_details()
	{
		...
		var buser=myForm.s_buser.value;
		var bemail=myForm.s_bemail.value;
		var x=new XMLHttpRequest();
				x.onreadystatechange=function()
				{
					if(x.readyState<4)
				...
				x.open("POST","institute_signup_data.py",true);
				x.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		x.send("s_user="+user+"&s_email="+email+"&s_buser="+buser+"&s_bemail="+bemail+"&s_password="+password+"&s_mobile="+mobile);
